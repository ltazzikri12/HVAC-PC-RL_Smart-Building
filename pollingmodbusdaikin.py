...
def read_group_data(start_address):
    modbus_address = start_address - 30001  # Konversi ke alamat 0-based
    result = client.read_input_registers(modbus_address, 6, unit=2)  # Pastikan unit ID benar
    if not result.isError():
        regs = result.registers
        print(f"Registers read: {table_name} : {regs}")  # Debugging: cek nilai register
...
while True:
    ...
    time.sleep(120)  # Waktu (sekon) sebelum polling berikutnya
